#!/usr/bin/bash

echo "Phase 05"
cd /home/ares/projects/BridgeLabz/Log_Analytics_System

echo "Running Phase 05"

echo "Making Custom Mapper and Reducer Executable"
chmod +x ./custom_wordcount/mapper_error.py ./custom_wordcount/reducer.py

echo "Removing old output (if exists)"
hdfs dfs -rm -r -f /Log_Analytics_System/output_py_wc

echo "Running Hadoop Streaming Job"

hadoop jar $HADOOP_HOME/share/hadoop/tools/lib/hadoop-streaming-3.5.0.jar \
  -files custom_wordcount/mapper_error.py,custom_wordcount/reducer.py \
  -input /Log_Analytics_System/large_logs/largefileV2.log -output /Log_Analytics_System/output_py_wc \
  -mapper mapper_error.py \
  -combiner reducer.py \
  -reducer reducer.py

echo "Verifying Output"
hdfs dfs -cat /Log_Analytics_System/output_py_wc/part-00000 | head

echo "Phase 05 Completed"
